...
This sealed hierarchy ensures type safety while allowing extensibility:

### Field-Level Method Generation

Generators can create individual methods for builder fields by implementing `appliesToField()` and `generateMethods()`.
...
Generators can modify the entire builder class after all field methods are generated by implementing `appliesToBuilder()` and `enhanceBuilder()`.
...
public class CustomValidationGenerator implements Generator {
    
    @Override
    public boolean appliesToField(FieldDto field, TypeName dtoType, ProcessingContext context) {
...
public class CustomValidationEnhancer implements Generator {
    
    @Override
    public boolean appliesToBuilder(BuilderDefinitionDto builderDto, TypeName dtoType, ProcessingContext context) {
...
```java
public class DateParserGenerator implements Generator {
...
```java
public class BuilderFactoryEnhancer implements Generator {
...
```java
public class BeanValidationEnhancer implements Generator {
...
```java
public class SpringBuilderEnhancer implements Generator {
