...
- **Lombok** — the closest "add a builder to my existing class" tool. The key difference is mechanism: instead of generating separate source, Lombok modifies your class during compilation by mutating the compiler's internal, non-public AST APIs. A standard annotation processor may only add new files, never alter existing ones, so this steps outside the normal model — it typically requires an IDE plugin to resolve the injected members, the generated code isn't visible as source (you run `delombok` to see it), and because it relies on those internal compiler APIs, each new JDK release generally needs a Lombok update before your project will compile — for example JDK 16's strong encapsulation of JDK internals ([JEP 396](https://openjdk.org/jeps/396)) broke Lombok until [v1.18.20](https://projectlombok.org/changelog), and the same "JDKxx support added" step recurs for JDK 17, 21, 23 and beyond. Lombok is a broad toolkit — getters/setters, `@Data`, `@Value`, `@SneakyThrows`, `@EqualsAndHashCode`, `@Delegate`, `val`, and more — and that breadth is genuinely useful when you want it, but its implicit behavior can be misused by less-experienced developers; for example, `@Data` or `@EqualsAndHashCode` on JPA entities can cause broken equality or lazy-loading pitfalls, while `@SneakyThrows` can silently bypass checked exceptions. Because Lombok injects code into the compiler AST rather than emitting source, those side effects are implicit and only visible through `delombok`, which can make subtle issues harder to debug. It has no runtime dependency and is widely used — choose Lombok if you're already invested in it for broader boilerplate reduction; choose Simple Builders when you prefer one focused capability — builders — with explicit, readable generated source.
