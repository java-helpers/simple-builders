...
class BuilderProcessorTest {
  ...
  // Tests for GeneratorRegistry error handling paths
  // These tests verify that the error handling code in GeneratorRegistry is functional

  @Test
  void shouldHandleProcessingErrorsGracefully() {
    // This test verifies that the processor can handle edge cases without crashing
    // It indirectly tests the error handling paths in GeneratorRegistry

    String packageName = "test";
    String className = "ErrorHandlingTest";
    String builderClassName = className + "Builder";
    ...
    // When: Compile with potentially problematic configuration
    Compilation compilation =
        ProcessorTestUtils.createCompiler()
            .withOptions("-Averbose=true") // Enable verbose to see error handling
            .compile(sourceFile);

    // Then: Should handle compilation gracefully
    String generatedCode = loadGeneratedSource(compilation, builderClassName);
    assertGenerationSucceeded(compilation, builderClassName, generatedCode);

    // Verify basic functionality still works
    ProcessorAsserts.assertContaining(
        generatedCode, "public " + builderClassName + " name(String name)");
    // Check for List field methods - the exact signature may vary
    ProcessorAsserts.assertContaining(
        generatedCode, "public " + builderClassName + " items(List<String> items)");
  }

  @Test
  void shouldContinueProcessingWithMultipleFields() {
    // This test verifies that processing continues even with multiple complex fields
    // which exercises the generator registry's error handling capabilities

    String packageName = "test";
    String className = "MultiFieldTest";
    String builderClassName = className + "Builder";
    ...
    JavaFileObject sourceFile =
        ProcessorTestUtils.simpleBuilderClass(
            ...
            """
                ...
                private java.util.Set<String> setField;
                private java.util.Map<String, String> mapField;
                ...
                public void setListField(java.util.List<String> listField) { this.listField = listField; }
                public java.util.Set<String> getSetField() { return setField; }
                public void setSetField(java.util.Set<String> setField) { this.setField = setField; }
                public java.util.Map<String, String> getMapField() { return mapField; }
                public void setMapField(java.util.Map<String, String> mapField) { this.mapField = mapField; }
            """);

    // When: Compile with multiple complex field types
    Compilation compilation =
        ProcessorTestUtils.createCompiler().withOptions("-Averbose=true").compile(sourceFile);

    // Then: Should handle all fields without errors
    String generatedCode = loadGeneratedSource(compilation, builderClassName);
    assertGenerationSucceeded(compilation, builderClassName, generatedCode);

    // Verify all field types were processed
    ProcessorAsserts.assertContaining(
        generatedCode, "public " + builderClassName + " simpleField(String simpleField)");
    ProcessorAsserts.assertContaining(
        generatedCode, "public " + builderClassName + " listField(List<String> listField)");
    ProcessorAsserts.assertContaining(
        generatedCode, "public " + builderClassName + " setField(Set<String> setField)");
    ProcessorAsserts.assertContaining(
        generatedCode, "public " + builderClassName + " mapField(Map<String, String> mapField)");
  }
}
